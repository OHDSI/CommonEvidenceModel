IF OBJECT_ID('@fqntableName','U') IS NOT NULL DROP TABLE @fqntableName;

CREATE TABLE @fqntableName  (

  {@targetDialect == "sql server"} ? {ID INT IDENTITY(1,1) PRIMARY KEY,}
  {@targetDialect == "postgresql"} ? {ID SERIAL PRIMARY KEY,}

	SOURCE_ID	VARCHAR(20),
	SOURCE_CODE_1	VARCHAR(256),   -- needs to be varchar(256) in order to hold a string of pipe separated CUIs
	SOURCE_CODE_TYPE_1	VARCHAR(55),
	SOURCE_CODE_NAME_1	VARCHAR(255),
	RELATIONSHIP_ID	VARCHAR(20),
	SOURCE_CODE_2	VARCHAR(256),   -- needs to be varchar(256) in order to hold a string of pipe separated CUIs
	SOURCE_CODE_TYPE_2	VARCHAR(55),
	SOURCE_CODE_NAME_2	VARCHAR(255),
	UNIQUE_IDENTIFIER	VARCHAR(50),
	UNIQUE_IDENTIFIER_TYPE	VARCHAR(50),
	PREDICATION_ID INT,
  PREDICATE VARCHAR(50),
  SUBJECT_SEMTYPE VARCHAR(50),
  SUBJECT_NOVELTY INT,
  OBJECT_NOVELTY  INT,
  SENTENCE_ID INT,
  "TYPE"  VARCHAR(5),
  "NUMBER"  INT,
  SENTENCE  VARCHAR(999),
  ISSN  VARCHAR(10),
  DP  VARCHAR(50),
  EDAT  VARCHAR(50),
  PYEAR INT
);

WITH CTE_SEMMEDDB AS (
	SELECT 'SEMMEDDB' AS SOURCE_ID,
		SUBJECT_CUI AS SOURCE_CODE_1,
		'UMLS CUI' AS SOURCE_CODE_TYPE_1,
		SUBJECT_NAME AS SOURCE_CODE_NAME_1,
		PREDICATE AS RELATIONSHIP_TYPE_ID,
		OBJECT_CUI AS SOURCE_CODE_2,
		'UMLS CUI' AS SOURCE_CODE_TYPE_2,
		OBJECT_NAME AS SOURCE_CODE_NAME_2,
		p.PMID AS UNIQUE_IDENTIFIER,
		'PMID' AS UNIQUE_IDENTIFIER_TYPE,
		PREDICATION_ID, PREDICATE, SUBJECT_SEMTYPE, SUBJECT_NOVELTY,
		OBJECT_SEMTYPE, OBJECT_NOVELTY,
		p.SENTENCE_ID, "TYPE", "NUMBER", SENTENCE,
		ISSN, DP, EDAT, PYEAR
	FROM @sourceSchema.PREDICATION p
		LEFT OUTER JOIN @sourceSchema.SENTENCE s
			ON s.SENTENCE_ID = p.SENTENCE_ID
		LEFT OUTER JOIN @sourceSchema.CITATIONS c
			ON p.PMID = c.PMID
	WHERE PREDICATE IN (
		'CAUSES','AFFECTS','ASSOCIATED_WITH',
		'COMPLICATES','DISRUPTS','INHIBITS',
		'INTERACTS_WITH','PREDISPOSES','PREVENTS',
		'PRODUCES','STIMULATES','TREATS',

		'NEG_CAUSES','NEG_AFFECTS','NEG_ASSOCIATED_WITH',
		'NEG_COMPLICATES','NEG_DISRUPTS','NEG_INHIBITS',
		'NEG_INTERACTS_WITH','NEG_PREDISPOSES','NEG_PREVENTS',
		'NEG_PRODUCES','NEG_STIMULATES','NEG_TREATS'
	)
	AND SUBJECT_SEMTYPE IN (
		/*DRUGS*/
		'clnd', /*Clinical Drug*/
		'phsu', /*pharmaceutical substance*/
		'antb', /*Antibiotic*/
		'bacs', /*Biologically Active Substance*/
		'orch', /*organic chemical*/
		'inch', /*Inorganic Chemical*/
		'horm', /*Hormone*/
		'vita' /*Vitamin*/
	)
	AND OBJECT_SEMTYPE IN (
		/*CONDITIONS*/
		'cgab', /*Congenital Abnormality*/
		'dsyn', /*Disease or Syndrome*/
		'mobd', /*Mental or Behavioral Dysfunction*/
		'patf', /*Pathologic Function*/
		'inpo', /*Injury or Poisoning*/
		'sosy', /*Sign or Symptom*/
		'rich', /*Rickettsia or Chlamydia*/
		'acab'	/*Acquired Abnormality*/
	)
)
INSERT INTO @fqntableName (SOURCE_ID, SOURCE_CODE_1,
	SOURCE_CODE_TYPE_1, SOURCE_CODE_NAME_1, RELATIONSHIP_ID,
	SOURCE_CODE_2, SOURCE_CODE_TYPE_2,	SOURCE_CODE_NAME_2,
	UNIQUE_IDENTIFIER, UNIQUE_IDENTIFIER_TYPE, PREDICATION_ID, PREDICATE, SUBJECT_SEMTYPE,
	SUBJECT_NOVELTY, OBJECT_NOVELTY, SENTENCE_ID, "TYPE", "NUMBER", SENTENCE,
	ISSN, DP, EDAT, PYEAR)
SELECT SOURCE_ID,
	SOURCE_CODE_1,
	SOURCE_CODE_TYPE_1,
	SOURCE_CODE_NAME_1,
	RELATIONSHIP_TYPE_ID,
	SOURCE_CODE_2,
	SOURCE_CODE_TYPE_2,
	SOURCE_CODE_NAME_2,
	UNIQUE_IDENTIFIER,
	UNIQUE_IDENTIFIER_TYPE,
	PREDICATION_ID,
	PREDICATE,
	SUBJECT_SEMTYPE,
	CAST(SUBJECT_NOVELTY as integer),
	CAST(OBJECT_NOVELTY as integer),
	SENTENCE_ID,
	"TYPE",
	"NUMBER",
	SENTENCE,
	ISSN,
	DP,
	EDAT,
	PYEAR
FROM CTE_SEMMEDDB;

CREATE INDEX IDX_@tableName_UNIQUE_IDENTIFIER_SOURCE_CODE_1_SOURCE_CODE_2 ON @fqntableName (UNIQUE_IDENTIFIER, SOURCE_CODE_1, SOURCE_CODE_2);

