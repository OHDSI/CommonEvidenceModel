...
loadSourceDefinitions <- function(schema,fileName) {
  tableName <- "SOURCE"
...
medlinePubmedFindMeshTags <- function(schema,sourceSchema,filter,justTableName=0){
...
cdmSTCM <- function(schema, fqTableName,vocabulary,umls) {
  ...
  #Create Table
  sql <- SqlRender::readSql("./SQL/CEM_SOURCE_TO_CONCEPT_MAP_createTable.sql")
  ...
  #Load UMLS CUI_TO_STANDARD code mappings
  sql <- SqlRender::readSql("./SQL/CEM_SOURCE_TO_CONCEPT_MAP_CUI_TO_STANDARD.sql")
  renderedSql <- SqlRender::renderSql(sql=sql,
                                      fqTableName=fqTableName,
  ...
  #Load MEDDRA_TO_STANDARD code mappings
  sql <- SqlRender::readSql("./SQL/CEM_SOURCE_TO_CONCEPT_MAP_MEDDRA_TO_STANDARD.sql")
  renderedSql <- SqlRender::renderSql(sql=sql,
                                      fqTableName=fqTableName,
                                      targetDialect=Sys.getenv("dbms"))
  translatedSql <- SqlRender::translateSql(renderedSql$sql,
  ...
  #Load MESH_TO_STANDARD code mappings
  sql <- SqlRender::readSql("./SQL/CEM_SOURCE_TO_CONCEPT_MAP_MESH_TO_STANDARD.sql")
  renderedSql <- SqlRender::renderSql(sql=sql,
                                      fqTableName=fqTableName,
                                      vocabulary=vocabulary,
                                      targetDialect=Sys.getenv("dbms"))
...
aeolusTranslate <- function(fqSourceTableName,fqTableName) {
  #connect
  connectionDetails <- DatabaseConnector::createConnectionDetails(
    dbms = Sys.getenv("dbms"),
    server = Sys.getenv("server"),
...
medlineAvillachTranslated <- function(fqSourceTableName,fqTableName) {
  #connect
  connectionDetails <- DatabaseConnector::createConnectionDetails(
    dbms = Sys.getenv("dbms"),
    server = Sys.getenv("server"),
...
medlinePubmedClean <-function(schema,sourceSchema,
                              #Do you want to find the mesh tag universe
                              pullMesh=1,
                              #do you want to pull from pubmed and where to start
                              pullPubMed=1,
                              pubMedPullStart=1,
                              #do you want to summarize pubmed and where to start
                              summarize=1,
                              summarizeStart=1){
  ...
  if(pullMesh == 1) {
    ...
    meshTagsTableName <- medlinePubmedFindMeshTags(schema="",
                                                   sourceSchema= "",
                                                   filter = 1,
                                                   justTableName=1)
  }
...
splicerTranlate <- function(fqSourceTableName,fqTableName){
  #connect
  connectionDetails <- DatabaseConnector::createConnectionDetails(
    dbms = Sys.getenv("dbms"),
    server = Sys.getenv("server"),
...
semMedDbTranslate  <- function(fqSourceTableName,fqTableName){
  #connect
  connectionDetails <- DatabaseConnector::createConnectionDetails(
    dbms = Sys.getenv("dbms"),
    server = Sys.getenv("server"),
...
euSplAdrClean<- function(schema,sourceSchema){
  tableName <- "EU_PL_ADR_CLEAN"
  fqntableName <- paste0(schema,".EU_PL_ADR_CLEAN")
...
euSplAdrTranslate  <- function(fqSourceTableName,fqTableName){
  #connect
  connectionDetails <- DatabaseConnector::createConnectionDetails(
    dbms = Sys.getenv("dbms"),
    server = Sys.getenv("server"),
