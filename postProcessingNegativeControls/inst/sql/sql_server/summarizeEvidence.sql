...
IF OBJECT_ID('tempdb..#TEMP_0', 'U') IS NOT NULL DROP TABLE #TEMP_0;
...
FROM @conceptUniverseData u
	LEFT OUTER JOIN @adeSummaryData descendant
		...
		AND descendant.MAPPING_TYPE = 'DESCENDANT_ALL'
...
SELECT t1.OUTCOME_OF_INTEREST_CONCEPT_ID, t1.OUTCOME_OF_INTEREST_CONCEPT_NAME,
  ...
  MAX(CASE WHEN t0.DATA_TYPE = 'MEDLINE_WINNENBURG' THEN t0.DESCENDANT_COUNT ELSE 0 END)  AS DESCENDANT_ALL_PMID_COUNT,
  ...
  MAX(CASE WHEN t0.DATA_TYPE = 'SPLICER' THEN 1 ELSE 0 END)  AS DESCENDANT_ALL_SPLICER,
  ...
  MAX(CASE WHEN t0.DATA_TYPE = 'AEOLUS' THEN 1 ELSE 0 END)  AS DESCENDANT_ALL_FAERS,
...
FROM #TEMP_1 t1
  JOIN #TEMP_0 t0
    ON t1.OUTCOME_OF_INTEREST_CONCEPT_ID = t0.OUTCOME_OF_INTEREST_CONCEPT_ID
  JOIN #TEMP_2 t2
