...
WITH CTE_LIST AS (
  ...
  CTE_KEEP_CONTROL AS (
	  SELECT l.CONCEPT_ID, l.CONCEPT_NAME, l.SORT_ORDER,
		MIN(
		  CASE
			  WHEN d.ROW_NUM = 1 THEN 1
			  WHEN d.ROW_NUM > 1 THEN 0  /*Don't keep Duplicates
			  ELSE 0
		  END
		  ) AS KEEP_CONTROL
	  FROM CTE_LIST l
  ...
  SELECT o.OUTCOME_OF_INTEREST_CONCEPT_ID,
	...
	o.USER_INCLUDED,
	CASE
		WHEN o.OPTIMIZED = 0 THEN 0
		WHEN c.KEEP_CONTROL IS NULL THEN o.OPTIMIZED
		WHEN o.OPTIMIZED = 0 THEN 0
		WHEN c.KEEP_CONTROL = 0 THEN 50
		WHEN c.KEEP_CONTROL = 1 THEN 60
		ELSE o.OPTIMIZED
	END
	 o.OPTIMIZED AS OPTIMIZED,
	NOT_PREVELANT
INTO @storeData
