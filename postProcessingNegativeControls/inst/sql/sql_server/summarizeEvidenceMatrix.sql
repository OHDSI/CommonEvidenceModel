...
IF OBJECT_ID('@evidence.TEMP_UNIVERSE', 'U') IS NOT NULL DROP TABLE @evidence.TEMP_UNIVERSE;
IF OBJECT_ID('@evidence.TEMP_CTE_PAIRS_EXPANDED', 'U') IS NOT NULL DROP TABLE @evidence.TEMP_CTE_PAIRS_EXPANDED;
IF OBJECT_ID('@evidence.TEMP_SUMMARIZE_EVIDENCE', 'U') IS NOT NULL DROP TABLE @evidence.TEMP_SUMMARIZE_EVIDENCE;


/*PREP - POSSIBLE PAIRS*/
SELECT *
INTO @evidence.TEMP_UNIVERSE
FROM @matrixConditions, @matrixIngredients;

CREATE INDEX IXD_TEMP_UNIVERSE ON @evidence.TEMP_UNIVERSE (INGREDIENT_CONCEPT_ID, CONDITION_CONCEPT_ID);


/*PREP - BUILD CONDITIONS OF INTEREST AND THEIR FAMILY ONCE AND INDEX*/
WITH CTE_PAIRS AS (
  SELECT *
  FROM @evidence.TEMP_UNIVERSE
),
CTE_PAIRS_EXPAND AS (
...
SELECT *
INTO @evidence.TEMP_CTE_PAIRS_EXPANDED
FROM CTE_PAIRS_EXPAND;

CREATE INDEX IXD_TEMP_CTE_PAIRS_EXPANDED ON @evidence.TEMP_CTE_PAIRS_EXPANDED (INGREDIENT_CONCEPT_ID, FAMILY_CONDITION_CONCEPT_ID);


/*SUMMARIZE WHERE EVIDENCE EXISTS - DROPPING LEFT OUTER JOIN HERE*/
WITH CTE_PAIRS_EXPAND AS (
	SELECT *
	FROM @evidence.TEMP_CTE_PAIRS_EXPANDED e
)
SELECT p.INGREDIENT_CONCEPT_ID, p.CONDITION_CONCEPT_ID,
	MAX(EVIDENCE_EXISTS) AS EVIDENCE_EXISTS
INTO @evidence.TEMP_SUMMARIZE_EVIDENCE
FROM (
	/*INDICATIONS*/
	SELECT p.INGREDIENT_CONCEPT_ID, p.CONDITION_CONCEPT_ID,
...
INTO @storeData
FROM CTE_UNIVERSE u
	LEFT OUTER JOIN @evidence.TEMP_SUMMARIZE_EVIDENCE e
...
CREATE INDEX IXD_MATRIX_SUMMARY ON @storeData (INGREDIENT_CONCEPT_ID, CONDITION_CONCEPT_ID);
